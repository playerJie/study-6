...
public class Test3 {
    ...
    void test1() {
        ...
        //Algorithm dbr = new DbrAlgo();
        Algorithm dbr = new AllAcoAlgo();
        Algorithm allAcoustic = new MyRouteAlgo2(dbr, dbr.getAlgoType(),
                "纯声", true,
                nodeConfig.getAcousticDistance(), nodeConfig.getAcousticDistance() + increment);
        Algorithm dss = new DssAlgo();
        Algorithm allOptical = new MyRouteAlgo2(dss, dss.getAlgoType(),
                "纯光", true,
                nodeConfig.getOpticalDistance(), nodeConfig.getOpticalDistance() + increment);
        Algorithm mix = new MyRouteAlgo();
        Algorithm mixed = new MyRouteAlgo2(mix, mix.getAlgoType(),
                "声光混合", true,
                nodeConfig.getOpticalDistance(), nodeConfig.getAcousticDistance() + increment);

        //List<Algorithm> algorithms = Arrays.asList(allAcoustic, allOptical, mixed);
        List<Algorithm> algorithms = Arrays.asList(allAcoustic);
        //List<Algorithm> algorithms = Arrays.asList(allOptical);
        //List<Algorithm> algorithms = Arrays.asList(mixed);
